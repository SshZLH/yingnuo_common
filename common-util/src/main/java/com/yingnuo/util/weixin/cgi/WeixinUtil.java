...
public class WeixinUtil {
    ...
    public static AccessToken getAccessToken(String appid, String secret) {
        ...
        if (cache != null) {
            ...
            String testUrl = CGI_GETCALLBACKIP_URL.replace("ACCESS_TOKEN", accessToken.getToken());
            JSONObject flag = httpRequest(testUrl, "GET", null);
            if (flag.get("errcode") == null) {
                logger.info("access_token_" + appid + "--未过期！");
                return accessToken;
            }
            removeAllCache("weixin");
        }
        logger.info("access_token_" + appid + "不存在或已过期！");
        ...
        if (null != jsonObject) {
            try {
                ...
                //access_token重新获取的同时刷新jsapi_ticket
                removeAllCache("weixin");
                getJsapiTicket(accessToken, appid, null);
            } catch (Exception e) {
                accessToken = null;
                e.printStackTrace();
                // 获取token失败
     ...
     * @return jsonObject
     */
    public static synchronized JSONObject sendTemplateMessage(String accessToken, String jsonStr) {
        logger.info("【通过accessToken发送模板消息】，PARAM:accessToken={},jsonStr={}", accessToken, jsonStr);
        String requestUrl = CGI_SEND_TEMPLATE_MESSAGE_URL.replace("ACCESS_TOKEN", accessToken);
        JSONObject jsonObject = httpRequest(requestUrl, "POST", jsonStr);
        if (null != jsonObject) {
            try {
                logger.info("【通过accessToken发送模板消息】，返回值：{}", jsonObject.toString());
            } catch (Exception e) {
                String error = String.format("发送模板消息失败 errcode:{} errmsg:{}", jsonObject.getInt("errcode"), jsonObject.toString());
                System.out.println(error);
                logger.error("【通过accessToken发送模板消息】，异常：{}", error);
            }
        }
        return jsonObject;
    }
    ...
    public static Map<String, String> getJsConfigMap(String url, String appId, String secret) {
        ...
        try {
            ...
            logger.error("缓存ticket获取失败----->", e.getMessage());
            removeAllCache("weixin");
            token = getAccessToken(appId, secret);
            jsonObject = getJsapiTicket(token, appId, url);
            logger.info("重新获取---->jsonObject------>" + jsonObject);
            jsapiTicket = jsonObject.getString("ticket");
        }
    ...
    public static JSONObject getJsapiTicket(AccessToken token, String appId, String url) {
        ...
        if (cache != null) {
            logger.info("jsapi_ticket_" + appId + "未过期！");
            return (JSONObject) cache;
        }
        logger.info("jsapi_ticket_" + appId + "不存在或已过期！");
        String requestUrl = CGI_JSAPI_TICKET.replace("ACCESS_TOKEN", token.getToken());
        JSONObject jsonObject = httpRequest(requestUrl, "GET", null);
        logger.info("getJsapiTicket-jsonObject:" + jsonObject.toString());
    ...
    public static void main(String args[]) {
        AccessToken accessToken = getAccessToken("wx266d92b77ba29197", "4be0cdc82ce4233a47c879f392405d1f");
